...
def test_no_view_function_found(sample_app):
    ...
    with pytest.raises(app.GruError):
...
def test_no_content_type_is_still_allowed():
    ...
    # to assume a default of application/json, so the gru handler needs
...
def test_exceptions_raised_as_app_errors(sample_app):
...
def test_app_view_errors_propagate_in_non_debug_mode(sample_app):
...
def test_app_view_errors_propagate_in_debug_mode(sample_app):
    @sample_app.route('/notfound')
    def notfound():
        raise NotFoundError("resource not found")
    sample_app.debug = True
...
def test_default_logging_handlers_created():
    ...
    # being explicit here.
    app.Gru('log_app', configure_logs=True)
...
def test_default_logging_only_added_once():
    ...
    app.Gru('added_once', configure_logs=True)
...
def test_logs_can_be_disabled():
    handlers_before = logging.getLogger('log_app').handlers[:]
    app.Gru('log_app', configure_logs=False)
    handlers_after = logging.getLogger('log_app').handlers[:]
    new_handlers = set(handlers_after) - set(handlers_before)
